...
parser.add_argument(
    '--file_generator', action='append', metavar=('output_prefix', 'executable'), nargs=2, default=[],
    help='File that when executed, outputs files')
...
def main(args):
    ...
    for item in args.file_generator:
        ...
        executable = executable.replace('${RUNFILES}', os.environ["RUNFILES"])
        try:
            subprocess.check_call([executable], cwd=tmpdir)
        ...
        for dirpath, dirnames, files in os.walk(tmpdir):
            ...
            if dirpath == tmpdir:
                ...
                relative_dirpath = dirpath[len(tmpdir):]
            else:
                raise ValueError("Unreachable")
            for filename in files:
                try:
                    put_file(args.output_dir + os.sep + prefix + os.sep + relative_dirpath + os.sep + filename,
                             src=tmpdir + os.sep + filename)
                except:
                    shutil.rmtree(tmpdir)
                    raise
        shutil.rmtree(tmpdir)
